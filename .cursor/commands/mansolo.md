...
3. For an explicit governed handoff, ensure `identity.actorId`, reuse or create the current session, and bind the existing TaskRef. If status has no current session, reuse an explicit session ID already retained in this conversation. Only if neither exists, run `mancode context session new --client cursor` exactly once and retain the returned session ID. Use `--client cursor` on every command that uses this session.
4. For that governed task only, read `mancode context show --purpose implement --session <id> --client cursor` using the bound or explicit TaskRef.
...
- For a governed-to-solo transition, use `mancode workflow handoff <namespace:ULID> --to solo --expected-revision <n> --session <id> --client cursor`.
