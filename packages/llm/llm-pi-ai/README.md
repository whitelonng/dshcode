...
A profile's `models` list *replaces* the route's installed catalog rather than extending it; omitting it (or leaving it empty) serves that catalog unchanged. Each entry defaults its unset fields from the installed model of the same `id`, so narrowing a catalog route to two models, correcting one capacity, or adding a model newer than the installed catalog are all one-line edits — but declaring any `models` list means every model the route should keep serving must appear in it, an entry of nothing but `id` being enough. The configurable entry fields are `id`, `name`, `contextWindow`, `maxTokens`, `input`, `output`, `capabilities`, `reasoningEfforts`, and `compat`. Pricing rides the installed entry or is absent; declared input and output modalities and capabilities flow to the harness seam (`LlmModelInfo`), which is what model selectors badge and image admission reads.
...
`output` declares the response modalities a model can produce; `image` is image generation, and absence means text-only output. `capabilities.imageUnderstanding` declares that the model can reason about image content — a stronger claim than merely accepting image input, and a model declaring it is expected to declare image input too, though nothing here forces the combination. Both fields are advisory metadata: the harness exposes them through `LlmModelInfo.outputModalities` and `LlmModelInfo.capabilities` for model selectors and capability surfaces, and this adapter's text seam never invokes image generation, so nothing beyond the declaration rides on them. Nothing inherits either field — pi-ai's `Model` type cannot carry them and the installed catalog declares no capabilities — so an entry that declares nothing resolves to the text-only floor and no capability claims.
...
A model **without** reasoning metadata — a catalog model pi-ai marks as non-reasoning, or a hand-declared entry that shadows such a catalog id and inherits the verdict — exposes no `reasoning` at all. pi-ai reports such a model as supporting the single level `off`, but `off` is translated to *omitting* the reasoning option, which is byte-for-byte the request that naming no effort already produces: selecting it could not disable anything, so a provider whose own default is to think would keep thinking with `off` shown as selected. Reporting the capability as unavailable leaves a surface offering the provider's default and nothing that misrepresents it. A hand-declared model that shadows no catalog entry instead defaults to the `off` / `low` / `max` offer when its entry declares no `reasoningEfforts`. The profile `reasoning` value, including `off`, is the deployment default when configured; omitting it preserves the provider default. Per-request `GenerateOptions.reasoningEffort` takes precedence, and a level absent from the exact model capability fails the REQUEST with `UNSUPPORTED_REASONING_EFFORT` before network I/O instead of being clamped. Describing a model never fails that way: the models under one provider disagree about which levels they accept, so `resolveModel` reports a profile level the exact model cannot take as no default at all rather than throwing. A throw there would take the whole provider out of every model catalog built over it — one mis-set profile field hiding even the models that do support the level — so a bad configuration surfaces where it is acted on, not where it is described. pi-ai's common stream options represent `off` by omitting `reasoning`.
