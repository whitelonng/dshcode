...
The wire face (`list` / `restore` / `remove`) is injected from `apply` and talks to the shared `/api` fetch carrier (`workspace.listArchived` / `workspace.restoreSession` / `workspace.deleteSession`), with responses validated by `protocol.ts` at the client boundary.
