...
import { DescribeImageCardController, type DescribeImageSettings } from '../src/client/describe-image-card-controller.ts'
...
describe('DescribeImageCardController', () => {
  it('reads the credential state for the reference the section names', async () => {
    ...
    const describe = vi.fn(() => Promise.resolve({
      rpcId: 'c-1' as never,
      result: { ok: true as const, value: { credentials: { VISION_API_KEY: { configured: true, writable: true } } } },
    }))
    const set = vi.fn(() => Promise.resolve({ rpcId: 'c-2' as never, result: { ok: true as const, value: {} } }))
    const controller = new DescribeImageCardController(host.scope, { credentials: { describe, set } } as never)
  ...
  it('writes the staged key through the credentials domain, never the settings section', async () => {
    const host = stubSettingsScope<DescribeImageSettings>()
    const credentials = credentialsApi(false)
    const controller = new DescribeImageCardController(host.scope, credentials.api)
    host.publish({ status: 'ready', writable: true, value: {}, user: {} })
    const face = controller.inject()

    face.edit('apiKey', ' vision-secret ')
    expect(face.hooks.describeImageCard.getSnapshot().dirty).toBe(true)
    expect(credentials.set).not.toHaveBeenCalled()
    ...
    expect(credentials.set).toHaveBeenCalledWith({ ref: 'VISION_API_KEY', value: 'vision-secret' })
    expect(host.set).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(face.hooks.describeImageCard.getSnapshot()).toMatchObject({ dirty: false, apiKeyConfigured: true })
    })
  })
  ...
  it('keeps the stored key when the draft is left blank', () => {
    ...
    expect(face.hooks.describeImageCard.getSnapshot().dirty).toBe(false)
    face.save()
  ...
  it('addresses the reference the section declares rather than the default', async () => {
    ...
    const credentials = credentialsApi(false)
    const controller = new DescribeImageCardController(host.scope, credentials.api)
    host.publish({ status: 'ready', writable: true, value: { apiKeyEnv: 'GLM_KEY' }, user: {} })
    const face = controller.inject()
    ...
    expect(credentials.set).toHaveBeenCalledWith({ ref: 'GLM_KEY', value: 'vision-secret' })
  })
  ...
  it('reports a key the Host did not store as a failed save', async () => {
    ...
    await vi.waitFor(() => {
      expect(face.hooks.describeImageCard.getSnapshot()).toMatchObject({ failed: true, dirty: true })
    })
  })
  ...
  it('keeps the card usable when the credential read fails', async () => {
    ...
    host.publish({ status: 'ready', writable: true, value: { model: 'glm-4v-flash' }, user: {} })
    face.edit('apiKey', 'vision-secret')
    face.save()
    await vi.waitFor(() => { expect(set).toHaveBeenCalled() })

    expect(face.hooks.describeImageCard.getSnapshot()).toMatchObject({
      available: true,
      apiKeyConfigured: false,
      model: { text: 'glm-4v-flash' },
    })
  })
  ...
  it('ignores a credential read the Host refused', async () => {
    ...
    const controller = new DescribeImageCardController(host.scope, { credentials: { describe, set: vi.fn() } } as never)
    await vi.waitFor(() => { expect(describe).toHaveBeenCalled() })
  ...
  it('saves the endpoint and the model together', async () => {
    ...
    face.edit('model', 'glm-4v-plus')
    face.save()
    await vi.waitFor(() => { expect(host.set).toHaveBeenCalledTimes(2) })

    expect(host.set.mock.calls).toEqual([['baseURL', 'https://other.test/v1'], ['model', 'glm-4v-plus']])
    expect(credentials.set).not.toHaveBeenCalled()
  })
})
