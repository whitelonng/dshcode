...
- **assistant 逐消息分页是预留 slot**：设计中已有图稿，尚未实现。已定稿的内容 IconActions 行（复制／时钟／分支／删除）只挂在每个已结束轮次中最后一条带 text 内容的 assistant 下；轮次中间的叙述、纯 Think 节点，以及仍在产出步骤的轮次里的所有节点都不带 chrome。除非该消息同时也是已完成轮次的最后一个 transcript 节点，否则分支保持禁用；启用后，它会 fork 到该轮次末尾，在 client 端递增继承标题并打开子会话。fork 或改名失败时源会话保持选中（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-02-message-fork-actions-require-completed-turn-tail.md)）。删除调用 `session.deleteMessage`，由 host 展开范围并追加 `message/delete` 事件；任意轮次运行中时删除按钮保持禁用，失败时以提示说明（[决策](../../../.agents/notes/implemented/feature/2026-08-16-message-deletion-and-transcript-removal.md)）。
- **user 消息可删除，最后一条可编辑**：user 气泡保留时钟、复制与删除（仅人类撰写的消息；轮次运行中禁用删除）；分支只存在于 assistant 回答之下（[决策](../../../.agents/notes/implemented/simplification/2026-08-06-user-bubbles-drop-the-branch-action.md)）。转录中最后一条人类用户消息在空闲时额外显示编辑：内联编辑器预填原文，提交调用 `session.editMessage`，重新生成轮次并阴影旧回答（[决策](../../../.agents/notes/implemented/feature/2026-08-16-user-message-edit-and-regenerate.md)）；回填 composer 输入框留待输入机引入编辑模式后再做。
