...
/**
 * Framework-wired session area (product render-prop surface). It follows the
 * current Session binding from the installed scope adapter and remounts the
 * body under `key={sessionId}` so a Session switch rebuilds the subtree. This
 * dependency-inverted layer uses plain string ids; the feature UI applies the
 * branded type at its own boundary.
 */
