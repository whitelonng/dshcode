...
describe('createFixtureApi', () => {
  ...
  it('workspace archive/restore round-trips the set and streams each change once', async () => {
    const api = createFixtureApi()
    const abort = new AbortController()
    const seen: HostFrame[] = []
    const consuming = (async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 10))
    const target = sid('fx-gamma')
    const archived = await api.workspace.archiveSession(req({ sessionId: target }))
    expect(archived.result).toMatchObject({ ok: true, value: { archivedSessionIds: [target] } })
    const listed = await api.workspace.listArchived(req({}))
    expect(listed.result).toMatchObject({ ok: true, value: { items: [{ sessionId: target }] } })
    // The idempotent repeat emits no second frame: restore's frame is next.
    await api.workspace.archiveSession(req({ sessionId: target }))
    const restored = await api.workspace.restoreSession(req({ sessionId: target }))
    expect(restored.result).toMatchObject({ ok: true, value: { archivedSessionIds: [] } })
    await consuming
    expect(seen).toEqual([
      { type: 'host/archived-sessions-changed', archivedSessionIds: [target] },
      { type: 'host/archived-sessions-changed', archivedSessionIds: [] },
    ])
  })

  it('workspace.deleteSession drops accounting and listing and announces the deletion', async () => {
    const api = createFixtureApi()
    const abort = new AbortController()
    const seen: HostFrame[] = []
    const consuming = (async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 10))
    const target = sid('fx-gamma')
    const unarchived = await api.workspace.deleteSession(req({ sessionId: target }))
    expect(unarchived.result).toMatchObject({ ok: false, error: { code: 'not-archived' } })

    await api.workspace.archiveSession(req({ sessionId: target }))
    const deleted = await api.workspace.deleteSession(req({ sessionId: target }))
    expect(deleted.result).toMatchObject({ ok: true, value: { archivedSessionIds: [] } })
    await consuming
    // The mirror-keeping increments: the setup archive's set change, then the
    // delete's emptied set, the owning workspace's pruned account, and the
    // deletion announcement itself.
    expect(seen[0]).toEqual({ type: 'host/archived-sessions-changed', archivedSessionIds: [target] })
    expect(seen[1]).toEqual({ type: 'host/archived-sessions-changed', archivedSessionIds: [] })
    expect(seen[2]).toMatchObject({
      type: 'host/workspace-changed',
      workspace: { workspaceId: 'fx-ws-fixture', sessionIds: ['fx-alpha', 'fx-beta'] },
    })
    expect(seen[3]).toEqual({ type: 'host/session-deleted', sessionId: target })

    const sessions = await api.sessions.list(req({}))
    if (!sessions.result.ok) throw new Error('session list failed')
    expect(sessions.result.value.items.map(session => session.sessionId)).not.toContain('fx-gamma')
    const listed = await api.workspace.listArchived(req({}))
