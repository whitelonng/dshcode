...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    host: {
      ...
      pickFiles: request => ok(request, { cancelled: false, paths: [`${FIXTURE_HOME}/Documents/upload.txt`] }),
      locateFiles: request => ok(request, { items: request.payload.names.map(name => ({ name, paths: [] })) }),
    },
    workspace: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'host.pickFiles': return this.api.host.pickFiles(request, new AbortController().signal)
      case 'host.locateFiles': return this.api.host.locateFiles(request, new AbortController().signal)
