...
- **删除 Workspace 注册记录不会删除 Session**：删除 Workspace 只移除注册记录本身，其 Session 的日志保留并移入 Ungrouped。删除对话走 Session 行的归档优先 Delete 与「归档会话」设置页（恢复与彻底删除）。
