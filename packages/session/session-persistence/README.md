...
| `delete(id): Promise<void>` | Durably delete one persisted session: drop in-memory state and remove the stored artifact. Resolves once the artifact, if any, is gone; the next `list` omits the id. Backends without `deleteStored` refuse loudly. |
