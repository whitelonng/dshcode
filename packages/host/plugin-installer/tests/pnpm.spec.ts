...
describe('installViaPnpm', () => {
  ...
  it('names the remedy when a workspace-protocol dependency blocks the install', async () => {
    const dir = await profileDir()
    answerPnpm('', '[ERR_PNPM_WORKSPACE_PKG_NOT_FOUND] In : "@deepseek-ai/schemastery@workspace:^" is in the dependencies but no package named "@deepseek-ai/schemastery" is present in the workspace', 1)
    await expect(installViaPnpm(dir, 'github:owner/demo'))
      .rejects.toThrow(/workspace:["']?-protocol dependencies[\s\S]*optional/)
  })
...
describe('readProfileIdentity', () => {
  ...
  it('rejects a pnpm placeholder manifest with the path-selector remedy', async () => {
    // pnpm installs a placeholder manifest when a git dependency's repository
    // root has no package.json; the diagnostic must name the remedy instead
    // of blaming the package name.
    const dir = await profileDir()
    await mkdir(join(dir, 'node_modules', 'monorepo-root'), { recursive: true })
    await writeFile(
      join(dir, 'node_modules', 'monorepo-root', 'package.json'),
      JSON.stringify({ _pnpmPlaceholder: 'This file was generated by pnpm. The original package did not contain a package.json.' }),
      'utf8',
    )
    await expect(readProfileIdentity(dir, 'monorepo-root'))
      .rejects.toThrow(/root has no package\.json[\s\S]*#&path:/)
  })
})
