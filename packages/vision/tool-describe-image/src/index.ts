 ...
 * Schemastery normalization, so every default and bound is re-judged here; the composition entry
 * calls this at load so misconfiguration fails loud.
 ...
 * over the composition entry and is re-resolved per call, so the Plugins card's changes reach the
 * very next invocation; the composition entry is still validated at load and fails loud.
