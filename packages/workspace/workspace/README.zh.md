...
- `ctx.workspaceRegistry.archiveSession(id)`/`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变，未来取消归档时可恢复原位置。归档接受任何实时或已持久化的会话（无论已记账还是 Ungrouped），对已归档的 id 直接完成而不写入，并拒绝未知 id。`restoreSession(id)` 从集合中移除一个 id（对未归档 id 幂等），`removeSession(id)` 把已删除的会话从每个所属 workspace 与集合中分离（未知 id 为 no-op）。在该字段出现之前写入的状态解析为一个空集合。
