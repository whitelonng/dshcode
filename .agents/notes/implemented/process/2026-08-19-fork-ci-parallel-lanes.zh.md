...
- `coverage` 以 `DSH_COVERAGE_MAX_WORKERS=3`（两个插桩 worker 加一个豁免重套件 worker）与 `DSH_GATE_CONCURRENCY=2` 运行，两个 coverage gate 以 2 + 1 = 3 个 fork 重叠而不是串行——按 4-vCPU 托管 runner 定尺寸。
