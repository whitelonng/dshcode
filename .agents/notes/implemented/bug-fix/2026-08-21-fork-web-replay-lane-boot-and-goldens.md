...
**The Plugins section's two tabs get distinct labels and orders.** Upstream's `ui-settings-plugin-inventory` (runtime fiber inventory) and the fork's `ui-settings-plugin-installer` (merged manage list) both registered `settings.plugins.tab` with the label 插件列表 / Plugin list. The inventory tab is renamed 插件状态 / Plugin status; the installer keeps 插件列表 because the settings e2e pins it as the manage surface. The installer also moves to `order: 20`: both shipped `order: 10`, and the section's stable sort then followed activation-order-dependent registration order, so the two tabs swapped positions across environments.
