...
> Scope: how the community dsh-web-ui feature plugins (task board, SSH panel, right panel, git graph, whale pet, mobile remote, live token stats) mount their GUI surfaces against this shell, and why the shell now declares two new slot seats plus a live-token projection read. Extends [Built-in community plugins and profile-scoped controls](2026-08-14-built-in-community-plugins-and-controls.md) (same distribution, this note covers the GUI mounting side); follows the [slot system standard](2026-07-22-slot-type-chain-implementation.md).

## Problem

The built-in `@linxin666/dsh-web-ui-all@0.1.2` plugins loaded server-side and their client bundles served, but none of the seven feature surfaces appeared in the GUI: the plugins mount at two kinds of points this shell did not provide. Task board, SSH, and the AionUi-style right panel wait on legacy DOM hooks (`[data-pane="sidebar"]`, `[data-pane="conversation"]`, `[data-dsh-frame]`) through MutationObservers, while git graph, the whale pet, and mobile remote register into slot seats (`conversation.input.selector.context`, `sidebar.remote`) that no entry declared. Live token stats registered a host projection (`liveTokenUsage`) but the conversation stats line never read it. Each missing point fails silently — an observer or `slots.inject` callback waits forever — so the deployment shows no error.
...
Two shell-owned slot seats close the slot-based gaps, following the slot system standard (children = declaration + authorization, one `register` API):

- `ui-sidebar` declares `'sidebar.remote'` (`single`, `root`, owner `{ wide }`) and renders it as its own foot row above the settings trigger, mirroring the `settingsArea` geometry in both wide and rail states. The mobile remote plugin's pairing trigger lands there.
...
**Re-register the family plugins against existing seats** (`sidebar.footer.action`, a new input-dock row) by rebuilding the whole family from source. Rejected: the published 0.1.2 bundles already target the two seats above, and porting the seats keeps the installed npm artifacts working without owning a second build pipeline.
...
The shipped shell now declares two additional root/session-maybe seats; unoccupied seats render nothing, so deployments without the community plugins see no layout change. The `liveTokenUsage` projection key is part of the standard projection map and its value arrives through the ordinary projection push channel — absent (older host or disabled plugin), the stats line simply omits the live group.
