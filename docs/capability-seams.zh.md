...
| `ctx.filePicker` | `seam` | `file-picker` | `file-picker-native` | `apiproxy` | - | 仅原生拾取缝隙：原生后端在 Host 显示设备上打开一个操作系统选择器并返回选中的绝对路径而不暂存字节；basename 定位辅助（./locate）遍历工作区树以解析拖拽的文件名，apiproxy 通过主机 RPC 面为两者提供服务。 |
